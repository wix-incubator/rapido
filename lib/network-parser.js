...
const parseRequestWillBeSent = ({ requestId, timestamp, request }, name) => {
  const { url, method, initialPriority } = request
  return { name, requestId, timestamp, url, method, initialPriority }
}

const parseResponseReceived = ({ requestId, type, response }, name) => {
  const { status: statusCode, fromDiskCache, fromServiceWorker, mimeType, url } = response
  return { name, type, requestId, statusCode, fromCache: fromDiskCache, fromServiceWorker, mimeType, url }
}

const parseLoadingFinished = ({ requestId, timestamp, encodedDataLength }, name) => ({ name, requestId, timestamp, encodedDataLength })
