...
const { isUrl, timlineFilter, moduleFilter, killProcess, getRemoteDebuggingPort } = require('./utils')
...
class Rapido {
  ...
  moduleSizeOf (mod) {
    const filter = moduleFilter(mod)
    const key = Object.keys(this.modules).filter(filter)[0]
