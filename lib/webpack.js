...
const run = (webpack, configPath = defaultConfig) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (config.filesystem) {
      compiler.outputFileSystem = config.filesystem
      compiler.inputFileSystem = config.filesystem
      compiler.resolvers.normal.fileSystem = config.filesystem
      compiler.resolvers.context.fileSystem = config.filesystem
    }
